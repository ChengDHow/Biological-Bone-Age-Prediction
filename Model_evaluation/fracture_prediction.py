...
if __name__ == "__main__":
    ...
    class fall(Dataset):
        ...
        def __init__(self):
            # Initialize data, download, etc.
            # read with numpy or pandas
            xy = np.loadtxt(r"D:\FYP\UK Biobank\fracture_onetp_scaled.csv", delimiter=',', dtype=np.float32, skiprows=1)
            self.n_samples = xy.shape[0]

            # Skip the first column which is just indexing. Second column is age and the rest are features.
            self.x_data = torch.from_numpy(xy[:, 1:40]) # size [n_samples, n_features]
    ...
    with torch.no_grad():
        ...
        mse=0
        predicted_age= []
        for i, (features, age) in enumerate(data_loader):
            features=features.reshape(-1, sequence_length, input_size).to(device)
            age=age.to(device)
            output,hidden=model(features)
            #output=torch.reshape(output,(len(output),sequence_length))
            mae+=(abs(age-output))
            predicted_age.append(output[0][0].numpy())

        mae = torch.sum(mae/len(data_loader))
        print(f'Mean absolute error of the model on the 18,167 test samples: {mae}')

    df = pd.read_csv('fracture_onetp_scaled.csv')
    df['predicted'] = predicted_age


    df.to_csv('fracture_onetp_pred_age.csv')
